...
![Single-brand departments in 2024](docs/figures/single_brand_markets.png)

*Every department that had an outlet selling to the public in 2024, drawn at the average position of its own outlets, since the panel carries coordinates for outlets and not boundaries. A department counts as single-brand when every outlet in it reports the same bandera. Eighty-two of the 441 are single-brand and 55 of those are YPF's, and the split barely moves over the period: year by year, between 54 and 64 departments are YPF's alone against 26 to 31 for every other brand together, unbranded outlets included, and YPF's share of them stays between 64 and 70 percent. This is also why the comparison below is lopsided: the large private brands are alone in only a fraction of that second group. Drawn by `code/figure_readme.R`.*
