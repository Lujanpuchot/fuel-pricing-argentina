...
In the model YPF maximizes profit plus $\lambda$ times consumer surplus, so its margin is $(1-\lambda)$ times the profit-maximizing margin. That one parameter carries the whole thesis, and the work is to pin down five things about it: how much margin the firm gives up, whether the answer moves with the government or with ownership, where the behaviour shows up (captive markets against competitive ones), through which channel it travels (the pump price or the wholesale price charged to its dealers), and what it is worth in pesos per litre to the firm and to consumers. The headline result is not the level of $\lambda$, which depends on cost components that are hard to measure, but YPF's excess weight relative to private brands selling in the same markets under the same cost shocks.
