...
One extension is worth naming, because the design leaves the door open for it. Regulation enters here as context: it defines the periods and it is described, not estimated. It is not given a parameter of its own, so the question of whether state control and price regulation are substitutes or complements cannot be answered yet. Answering it needs a credible weight on regulation alongside $\lambda$, and the sample offers a way to get one. Between thirty and forty departments a year have no YPF station at all. Regulation binds there as everywhere else, but no state-owned firm is present to hold prices down, so the compression of private margins in those markets identifies the effect of regulation on its own, against the markets where the two act together. I have not estimated this and it is not part of the thesis as it stands.
